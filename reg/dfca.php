<?php
    ...
    if($con){
        if($_POST){
            ...
            $mq="UPDATE `ktumarks` SET `Series_1` = '$smark1', `Series_2` = '$smark2', `Assignment_1` = '$assign1', `Assignment_2` = '$assign2', `Attendance` = '$attend' WHERE `ktumarks`.`KTU_ID` = 'TVE23MCA-2035' and `Subject` = 'DFCA';";
            $me=mysqli_query($con,$mq);  
            $mm="UPDATE `ktumarks` SET `Internal` = '$int' where `KTU_ID` = 'TVE23MCA-2035' and `Subject`='DFCA';";
            $mmq=mysqli_query($con,$mm);
        }
        $sq="select * from ktumarks where KTU_Id='TVE23MCA-2035' and Subject ='DFCA'";
